...
namespace U3DUtility
{
    ...
    public class TcpLayer : MonoBehaviour
    {
        ...
        {
            //注册DLL用到的委托类型
            domain.DelegateManager.RegisterDelegateConvertor<U3DUtility.TcpLayer.OnConnectEvent>((act) =>
            {
                return new U3DUtility.TcpLayer.OnConnectEvent((isSuccess, msg) =>
                {
                    ((System.Action<System.Boolean, System.String>)act)(isSuccess, msg);
                });
            });

            domain.DelegateManager.RegisterDelegateConvertor<U3DUtility.TcpLayer.OnDisconnectEvent>((act) =>
            {
                return new U3DUtility.TcpLayer.OnDisconnectEvent((msg) =>
                {
                    ((System.Action<System.String>)act)(msg);
                });
            });

            domain.DelegateManager.RegisterDelegateConvertor<U3DUtility.TcpLayer.OnRecvEvent>((act) =>
            {
                return new U3DUtility.TcpLayer.OnRecvEvent((id, data) =>
                {
                    ((System.Action<int, byte[]>)act)(id, data);
                });
            });

            domain.DelegateManager.RegisterMethodDelegate<int, byte[]>();
            domain.DelegateManager.RegisterMethodDelegate<System.Boolean, System.String>();
            domain.DelegateManager.RegisterMethodDelegate<System.String>();
        }
        ...
        public void SendPack(ushort messId, byte[] data)
        {
            ...
            binaryWriter.Write(data.Length + MSG_ID_SIZE); //写入一个int，表示包大小
            binaryWriter.Write(messId); //消息id
            binaryWriter.Write(data, 0, (int)data.Length);  //序列化之后的消息体
        ...
        void OnConnectCallback(IAsyncResult asyncResult)
        {
            ...
            finally
            {
                ...
                Loom.QueueOnMainThread(() => //到主线程中通知连接成功
                {
                    if (IsInvoking("ConnectTimeOutCheck"))
                    {
                        CancelInvoke("ConnectTimeOutCheck");
        ...
        void ReadAsyncCallBackPackHead(IAsyncResult asyncResult)
        {
            try
            {
                ...
                if (dataLen == 0)
                {
                    Loom.QueueOnMainThread(() =>
                    {
                        Disconnect("recv 0 bytes and server disconnected");
                    });
